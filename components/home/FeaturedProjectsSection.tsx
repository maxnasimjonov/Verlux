      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        <ScrollAnimation direction="up" delay={0.1}>
          <div className="flex justify-between items-end mb-20">
            <div>
              <div className="text-sm uppercase tracking-[0.2em] text-gray-600 font-bold mb-4">Featured Work</div>
        ...
        >
          {projects.map((project, index) => (
            <div
              key={index}
              className="group relative overflow-hidden bg-white border-2 border-gray-200 rounded-3xl min-h-[500px] hover:border-gray-600 transition-all duration-500 cursor-pointer hover:shadow-2xl"
            >
              {/* Project Image */}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 z-20">
                <div className="flex items-center justify-between mb-4">
                  <span className="px-4 py-2 bg-gray-600 text-white rounded-full text-sm font-bold uppercase tracking-wider">
                    {project.category}
                  </span>
                  <span className="text-white/60 text-sm font-semibold">{project.year}</span>
                </div>
                <h3 className="text-4xl font-black mb-2 text-white group-hover:text-gray-500 transition-colors">
                  {project.title}
                </h3>
